...
(function () {
  ...
  function getFontParts (input) {
    ...
    var parts = font.split(oneOrMoreSpaces);
    ...
    while (parts.length > 2) {
      if (isFontStyle.exec(parts[0])) {
        fontStyle = parts.splice(0, 1)[0];
      } else if (isFontVariant.exec(parts[0])) {
        fontVariant = parts.splice(0, 1)[0];
      } else if (isFontWeight.exec(parts[0])) {
        fontWeight = parts.splice(0, 1)[0];
      } else if (isNormal.exec(parts[0])) {
        parts.splice(0, 1);
      } else {
        return defaultFont;
      }
    ...
    return [
      ...
      parts.splice(0, 1)[0] || defaultFont[3],
      parts.join(' ') || defaultFont[4]
    ];
  }
