...
(function () {
  ...
  var incorrectPointFormat = 'Path points must be an array of numbers [x, y, x, y], pairs [[x, y], [x, y]], or objects [{x, y}, {x, y}].';
  ...
  function Canvas (input) {
    ...
    this.plotPath = function (points) {
      ...
      if (typeof transformedPoints[0] === 'number') {
        if (transformedPoints.length === 2) {
          return this;
        }

        increment = 2;
        this.moveTo(transformedPoints[0], transformedPoints[1]);
      } else {
        this.moveTo(transformedPoints[0][0], transformedPoints[0][1]);
      }

      for (var i = increment; i < transformedPoints.length; i += increment) {
        if (increment === 2) {
          this.lineTo(transformedPoints[i], transformedPoints[i + 1]);
        } else {
          this.lineTo(transformedPoints[i][0], transformedPoints[i][1]);
        }
      }
    ...
    this.fillPoint = function (x, y, color) {
      return this
        .fillRect(x, y, 1, 1, color);
    }.bind(this);

    this.strokePoint = function (x, y, color) {
      return this
